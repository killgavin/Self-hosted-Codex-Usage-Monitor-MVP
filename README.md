...
returned by the monitor API. Dashboard data is available only while the
official Codex/ChatGPT account is logged in.
>>>>>>> 10ddbee (feat: gate dashboard with Codex login session)
...
monitor. The official Codex/ChatGPT login gates protected dashboard routes. The Compose setup publishes to loopback by
